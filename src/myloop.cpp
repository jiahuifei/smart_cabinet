...
static lv_obj_t * busy_dialog = NULL;
...
// 创建带加载动画的模态对话框
void show_blocking_dialog(const char* msg) {
  // 创建模态背景
  lv_obj_t * modal = lv_obj_create(lv_scr_act());
  lv_obj_set_size(modal, LV_PCT(100), LV_PCT(100));
  lv_obj_set_style_bg_opa(modal, LV_OPA_50, 0);
  lv_obj_set_style_bg_color(modal, lv_color_hex(0x000000), 0);
  lv_obj_add_flag(modal, LV_OBJ_FLAG_CLICKABLE); // 阻止点击穿透

  // 创建对话框主体
  busy_dialog = lv_obj_create(modal);
  lv_obj_set_size(busy_dialog, 300, 200);
  lv_obj_center(busy_dialog);
  
  // 添加消息文本
  lv_obj_t * label = lv_label_create(busy_dialog);
  lv_label_set_text(label, msg);
  lv_obj_align(label, LV_ALIGN_TOP_MID, 0, 20);

  // 添加加载动画
  lv_obj_t * spinner = lv_spinner_create(busy_dialog, 1000, 60);
...
TagData ReadTagData(int inx)
{
  ...
  if (isRfid1(inx))
  {
    ret = rfid1.cmd_read_tag(inx);
    // 如果成功则，读取具体tag数据
    if (ret)
    {
      currentTag = rfid1.read_tag_data();
  ...
  {
    ret = rfid2.cmd_read_tag(inx);
    if (ret)
    {
      currentTag = rfid2.read_tag_data();
    }
    else
    {
    }
...
bool DataUpdate(int cabinetId, bool statu)
{
  Serial.print("Update : Update goods , cabinetId:");
  Serial.print(cabinetId);
  Serial.print("；状态：");
  Serial.println(statu ? "有物资":"无物资");
  ...
  if (cabinetType == CABINET_TYPE_TRIPLE)
  {
    Serial.println("Update : 三物资柜子");
    // 数据库更新同步
    TripleCabinetItem *currentItem = ItemDatabase::getTripleCabinetInfo(cabinetId);
    if (currentItem == NULL)
      return false; // 无效格口ID


      // 更新状态 ??? 如何判断物资索引
    currentItem->itemActualStatus1 = (statu ? 1 : 0);
  }
  else if (cabinetType == CABINET_TYPE_SINGLE)
  {
    Serial.println("Update : 单物资柜子");

    // 单物品格口处理
    SingleCabinetItem *currentItem = ItemDatabase::getSingleCabinetInfo(cabinetId);
    if (currentItem == NULL)
      return false; // 无效格口ID

    currentItem->itemActualStatus = statu; // 更新物品状态
  }
  else{
    Serial.println("Update : 未知状态");
  }
...
void CheckDoorState()
{
  ...
  show_blocking_dialog("Processing...");
  ...
  for (int inx = 0; inx < 48; inx++)
  {
    ...
    Serial.print("Update : Check Data ,current Index =");
    ...
    if (!retItem)
    {
      Serial.println("Update : Current Index Skip");
      continue;
    }
    else{
    ...
    */
    ...
    if (ret)
    {
      ...
      {
        Serial.println("Update : Door unlock ,Set warning flag");
        mIsWarning = true;
      }
    ...
    // 根据不同场景判断判断tag数据的合法性
    // 物资被取走，对应物资类别能匹配
    if (currentOperationType == STATUS_BORROW)
    {
      if (currentTag.valid)
      {
        Serial.println("Update : Borrow, Find goods exist");
        mIsWarning = true; // 物资未被取走，触发告警
      }
    }
    else if (currentOperationType == STATUS_RETURN)
    { // 对应物资类别应该被归还
      if (!currentTag.valid)
      {
        Serial.println("Update : Return, Find goods not exist");
        mIsWarning = true; // 物资未归还，触发告警
      }
    }
    else if (currentOperationType == STATUS_MAINTAIN)
    { // 无状态检测，需要进行数据同步
    }
    ...
    Serial.println("Update : ============更新数据===========Start===================");
    DataUpdate(inx,currentTag.valid);
    Serial.println("Update : ============更新数据===========End===================");
  }

  // 关闭对话框
  Serial.println("Update : Close model dialog");
  // 关闭对话框
  lv_obj_del(lv_obj_get_parent(busy_dialog)); // 删除模态层
  busy_dialog = NULL;

  if (mIsWarning)
  {
    // 声光报警，
    Serial.println("Update : 需要发出声光报警");

    return;
  }

  // 回到主页
  Serial.println("Update : 回到首页");
...
void processBorrowWorkflow()
{
  ...
  case 6: // 等待用户关闭格口
  {
    ...
    // 检查是否超时
    if (millis() - processStartTime > 30000)
    { // 30秒超时
...
void super_loop()
{
  ...
  if (lv_scr_act() == objects.manage)
  {
    ...
    for (int i = 0; i <= 48; i++)
    {
      ...
      {
        Serial.println("Update : 准备开锁");

        bool ret = directOpenLockById(i); // 按钮编号直接对应锁ID

        Serial.println("Update : 准备开锁完成");

        isControlIDS[i] = true;
        if (ret)
        {
          Serial.println("Update : 开锁成功---");
          isControlIDS[i] = ret;
        }
        else{
          Serial.println("Update : 开锁失败---");
        }
...
{
  if (!progress_msgbox || !progress_slider || !progress_label)
    return;
